...
function getJetstreamUrl(host: string, actorDid: string) {
  ...
  url.searchParams.append('wantedCollections', 'org.owdproject.application.desktop')
  url.searchParams.append('wantedCollections', 'org.owdproject.application.windows')
  url.searchParams.append('wantedCollections', 'org.owdproject.application.meta')
  return url.toString()
}
...
export async function startActorDesktopStreamw(actorDid: string) {
  ...
  ws = new WebSocket(getJetstreamUrl(runtimeConfig.public.atprotoJetstream.host, actorDid))
